...
const envPaths = [
    path.resolve(__dirname, '../.env'), // server/.env (if running from dist)
...
const connectedClients = new Map<string, { userId: string, nickname: string, assignment: string, sessionId?: number }>();
...
function getAssignment(userId: string): "CONTROL" | "TREATMENT" {
...
function getOnlineUsers() {
    const uniqueUsersMap = new Map<string, { userId: string, nickname: string, assignment: string }>();
    ...
    return Array.from(uniqueUsersMap.values()).map(u => ({
        userId: u.userId,
        nickname: u.nickname,
        assignment: u.assignment as "CONTROL" | "TREATMENT"
    }));
}
...
io.on('connection', (socket) => {
    ...
    socket.on('session:start', async (payload: unknown) => {
        try {
            ...
            const lastMessages = lastMessagesRaw.reverse().map((m: any) => ({
                id: m.id,
                userId: m.userId,
    ...
    socket.on('message:send', async (payload: unknown) => {
        try {
            ...
            try {
                ...
                if (response.ok) {
                    const data = await response.json() as { mood: string, intensity: number };
